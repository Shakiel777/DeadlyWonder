...
public class HFPSMenu : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("Create", GUILayout.Width(100), GUILayout.Height(30)))
        {
            ...
            AssetDatabase.CreateAsset(asset, "Assets/Horror FPS Kit/HFPS Assets/Scriptables/" + "SaveLoadSettings" + ".asset");
    ...
    private static void CreateAssetFile<T>(string AssetName) where T : ScriptableObject
    {
        ...
        ProjectWindowUtil.CreateAsset(asset, "Assets/Horror FPS Kit/HFPS Assets/Scriptables/New " + AssetName + ".asset");
    }
    ...
    {
        string path = "Assets/Horror FPS Kit/HFPS Assets/Scriptables/";

        if (Directory.Exists(path))
        {
            if (Directory.GetFiles(path).Length > 0)
            {
                return JsonManager.FolderPath(AssetDatabase.LoadAssetAtPath<SaveLoadScriptable>(path + "SaveLoadSettings.asset").filePath);
            }
            return JsonManager.FolderPath(FilePath.GameDataPath);
        }
        else
        {
            return JsonManager.FolderPath(FilePath.GameDataPath);
