...
public class Footsteps : MonoBehaviour
{
	...
	{
		soundsGO.PlayOneShot(m_Footsteps[1].footstep[Randomizer.Range(0, m_Footsteps[1].footstep.Length)], audioVolumeWalk);
	}
    ...
    public IEnumerator JumpLand()
    {
		...
		for (int i = 0; i < m_Footsteps.Length; i++)
		{
			if (curMat == m_Footsteps[i].groundTag) 
			{
				soundsGO.PlayOneShot(m_Footsteps[i].footstep[Randomizer.Range(0, m_Footsteps[i].footstep.Length)], 0.5f);
				yield return new WaitForSeconds(0.12f);
				soundsGO.PlayOneShot(m_Footsteps[i].footstep[Randomizer.Range(0, m_Footsteps[i].footstep.Length)], 0.4f);
			}
		}
    }
	...
	IEnumerator WalkOnGround()
	{
		for (int i = 0; i < m_Footsteps.Length; i++)
		{
			...
			{
				step = false;
				soundsGO.PlayOneShot(m_Footsteps[i].footstep[Randomizer.Range(0, m_Footsteps[i].footstep.Length)], audioVolumeWalk);
                if (!realStepWalk)
                {
    ...
    IEnumerator RunOnGround()
	{
		for (int i = 0; i < m_Footsteps.Length; i++)
		{
			...
			{
				step = false;
				soundsGO.PlayOneShot(m_Footsteps[i].footstep[Randomizer.Range(0, m_Footsteps[i].footstep.Length)], audioVolumeRun);
                if (!realStepRun)
                {
    ...
    IEnumerator CrouchOnGrouund()
    {
        for (int i = 0; i < m_Footsteps.Length; i++)
        {
            ...
            {
                step = false;
                soundsGO.PlayOneShot(m_Footsteps[i].footstep[Randomizer.Range(0, m_Footsteps[i].footstep.Length)], audioVolumeCrouch);
                if (!realStepCrouch)
                {
    ...
    IEnumerator WalkOnWater()
    {
        for (int i = 0; i < m_Footsteps.Length; i++)
        {
            ...
            {
                step = false;
                soundsGO.PlayOneShot(m_Footsteps[i].footstep[Randomizer.Range(0, m_Footsteps[i].footstep.Length)], audioVolumeWater);
                yield return new WaitUntil(() => !soundsGO.isPlaying);
                step = true;
            }
        }
