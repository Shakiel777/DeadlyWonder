/*
 * Inventory.cs - wirted by ThunderWire Games
 * ver. 1.3
*/
...
public class Inventory : MonoBehaviour, IPointerClickHandler {
    ...
    void Awake()
    {
        ...
        {
            Items.Add(new Item(i, inventoryDatabase.ItemDatabase[i]));
        }
    }
    ...
    {
        Item itemToAdd = GetItemByID(id);
        if (CheckInventorySpace() || CheckItemIDInventory(id))
        {
            for (int i = 0; i < slots.Count; i++)
            {
                if (i == slotID)
    ...
    public void RemoveAllItemAmounts(int id)
    {
        Item itemRemove = GetItemByID(id);
    ...
    public bool CheckSWIDInventory(int switcherID)
    {
        for (int i = 0; i < items.Count; i++)
        {
            if (items[i].useSwitcherID == switcherID)
            {
                return true;
            }
        }
    ...
    public void CombineWith(Item SecondItem, int slotID)
    {
        if (slotID != selectedID)
        {
            ...
            {
                int switcherID = GetItemByID(SelectedItem.combineSettings[0].combineWithID).useSwitcherID;
