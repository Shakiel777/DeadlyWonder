...
public class SaveGameHandler : MonoBehaviour {
    ...
    private void LoadSavedSceneData(bool allData)
    {
        ...
        {
            File.Delete(JsonManager.FolderPath(FilePath.GameDataPath) + "_nextSceneData.dat");
        }
    }

    /* LOAD SECTION INVENTORY */
    private System.Collections.IEnumerator LoadInventory(JToken token)
    {
        ...
        for (int i = 0; i < GetComponent<Inventory>().slots.Count; i++)
        {
            ...
            if (slotString != "null")
            {
                GetComponent<Inventory>().AddSlotItem((int)slotToken["slotID"], (int)slotToken["itemID"], (int)slotToken["itemAmount"]);
            }
        }
    }
    ...
    void SaveGame(bool betweenScenes)
    {
        string filepath = JsonManager.FolderPath(FilePath.GameSavesPath);
        ...
        if (!betweenScenes)
        {
            if (Directory.Exists(filepath))
            {
                ...
                if (fi.Length > 0)
                {
                    ...
                    JsonManager.SerializeData(SaveName + ".sav");
                }
                else
                {
                    lastSave = "Save0.sav";
                    JsonManager.SerializeData("Save0.sav");
                }
            }
            else
            {
        ...
        {
            JsonManager.SetFilePath(FilePath.GameDataPath);
            JsonManager.SerializeData("_nextSceneData.dat");

            FileInfo fi = JsonManager.GetSerializeInfo();
